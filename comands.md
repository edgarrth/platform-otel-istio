...
--------------
    ...
    helm repo update
    helm upgrade opentelemetry-operator open-telemetry/opentelemetry-operator --version 0.110.0 --install -n operators --create-namespace -f stack/otel/values-operator.yml
...
helm repo add prometheus-community https://prometheus-community.github.io/helm-charts
helm repo update
helm upgrade --install prometheus prometheus-community/prometheus -n prometheus --create-namespace --version 29.6.0 -f stack/prometheus/values-server.yaml
...
------
    ...
    sidecar:
    --------
        k label ns default istio.io/dataplane-mode-
        k label ns default istio.io/use-waypoint-
        k label ns default istio.io/use-waypoint-namespace-

        k label namespace default istio-injection=enabled
        -> revisar anotaciones entre opentelemetry & istio (del ns inyecta init container & sidecar)

alias kp="k -n prometheus"
kp port-forward svc/prometheus-server 9090:80
k label namespace default istio-injection=enabled
k logs -f frontend-7d5546bff7-jh57f -c istio-proxy

k run nginx --image nginx
k create ns test
k -n test nginx --image nginx
k exec -it nginx -- bash
curl -I https://api.ipify.org
curl -i https://api.ipify.org?format=json
k -n test expose pod nginx --port=80 --target-port=80 --name=nginx